# https://github.com/erprogs/CViT/tree/main?tab=readme-ov-file
...
def predict(
    ...
    if len(df) >= 1:
        with autocast(enabled=fp16):  # Enable mixed precision if fp16 is True
            y, y_val = pred_vid(df, model)
    else:
        y, y_val = 0, 0.5  # Default prediction for no face detected
...
def main():
    ...
    # Assuming `gen_parser` and the functions for dataset processing are defined elsewhere
    ...
    file_path = os.path.join("result", f"prediction.json")

    # Ensure the result directory exists
    os.makedirs(os.path.dirname(file_path), exist_ok=True)

    # Create the file before writing results
